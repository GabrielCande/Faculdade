...
//Função Principal
int main() {
  //Declara uma variável que irá receber as informações do arquivo binário e uma string que receberá o nome do arquivo
  int resultado;
  char arquivo[20];
  ...
  for(int i = 0; i < resultado; i++){
    ...
    switch(opcode){
      ...
      break;
      //---------------------------------- SW ---------------------------------------
      //Caso opcode possua o valor 0x2b
      case 0x2b:
        //Exibe as informações da cada variável
        printf("\n opcode %01x | regFont %01x | regTemp %01x | immediate %01x\n",
        opcode, regFont, regTemp, immediate);
        printf("\n----------SW---------\n");
        //Registrador fonte recebe TAM que aponta para a base da pilha
        R[regFont] = TAM;
        //Offset vira negativo para manipulação da pilha
        immediate = immediate * -1;
        //R[regTemp] recebe o valor retornado da função pegaValor
        R[regTemp] = pegaValor();
        //Se o R[regFont] + immediate for maior ou igual a TAM ou menor ou igual a quantidade de informações lidas
        if ((R[regFont] + immediate) >= R[regFont] || (R[regFont] + immediate) <= resultado) {
          //Exibe a mensagem de que a pilha estourou
          printf("Estorou a pilha!!\n");
          //Se não
        }else {
          //A ram na posição R[regFont] + immediate recebe o valor do registador temporário (R[regTemp])
          ram[R[regFont] + immediate] = R[regTemp];
          //Exibe o valor de R[regTemp]
          printf("A operação de transferência de dados SW resultou em %d\n", ram[R[regFont] + immediate]);
        }//if
      break;
      //---------------------------------- LW ---------------------------------------
      //Caso opcode possua o valor 0x23
      case 0x23:
        //Exibe as informações da cada variável
        printf("\n opcode %01x | regFont %01x | regTemp %01x | immediate %01x\n",
        opcode, regFont, regTemp, immediate);
        printf("\n----------LW---------\n");
        //Registrador regFont recebe TAM que aponta para a base da pilha
        R[regFont] = TAM;
        //Offset vira negativo para manipulação da pilha
        immediate = immediate * -1;
        //Se o R[regFont] + immediate for maior ou igual a TAM ou menor ou igual a quantidade de informações lidas
        if ((R[regFont] + immediate) >= R[regFont] || (R[regFont] + immediate) <= resultado) {
          ...
          //O registrador temporário (R[regTemp]) recebe o valor da memória ram na posição R[regFont] + immediate
          R[regTemp] = ram[R[regFont] + immediate];
          //Exibe o valor de R[regTemp]
          printf("A operação de transferência de dados LW resultou em %d\n", R[regTemp]);
        }//if
      break;
    }//switch
  }//for
